...
import { CreateRoomPayload, JoinRoomPayload, LeaveRoomPayload, PlayCardPayload, StartGamePayload, PlayerStatus, Message } from "./types/ServerTypes";
...
export function setupSocket(io: Server) {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("update_player_status", (payload: { roomId: string; status: PlayerStatus }) => {
    ...
    socket.on("send_message", (payload: { roomId: string; message: Message }) => {
    ...
    socket.on("respond_to_join_request", (payload: { requestId: string; accepted: boolean }) => {
