...
export function createRoom(
  io: Server,
  socket: Socket,
  { playerName, roomName }: CreateRoomPayload
) {
...
export function requestJoinRoom(
  io: Server,
  socket: Socket,
  { roomId, playerName }: JoinRoomPayload
) {
  if (state.socketRoomMap[socket.id]) {
...
export function respondToJoinRequest(
  ...
  if (room && room.ownerId === socket.id) {
    ...
    const userSocket = io.sockets.sockets.get(request.requesterSocketId);
    if (!userSocket) {
      socket.emit("response_error", {
    ...
    if (accepted) {
      ...
      state.socketRoomMap[request.requesterSocketId] = request.roomId;
...
export function kickPlayer(
  io: Server,
  socket: Socket,
  { playerToKickId }: { playerToKickId: string }
) {
...
export function updatePlayerStatusHandler(
  io: Server,
  socket: Socket,
